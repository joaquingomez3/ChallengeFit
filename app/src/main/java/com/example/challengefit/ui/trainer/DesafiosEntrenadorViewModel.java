...
public class DesafiosEntrenadorViewModel extends AndroidViewModel {
    private MutableLiveData<List<Desafio>> mLista;

    public DesafiosEntrenadorViewModel(@NonNull Application application) {
        super(application);
    }
    ...
    public void cargarDesafios(){
        ...
        if(token != null){
            ...
            Call<List<Desafio>> llamada = api.obtenerDesafios(token);
            ...
            llamada.enqueue(new Callback<List<Desafio>>() {
                ...
                public void onResponse(Call<List<Desafio>> call, Response<List<Desafio>> response) {
                    if(response.isSuccessful() && response.body() != null){
                        ...
                        Log.e("DesafiosVM", "Error en respuesta: " + response.code());
                    }
                }

                @Override
                public void onFailure(Call<List<Desafio>> call, Throwable t) {
                    Log.e("DesafiosVM", "Fallo conexión: " + t.getMessage());
                }
            });
        }
    }
